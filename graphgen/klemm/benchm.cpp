...
int klemm_benchmark(int num_nodes, int clique_size, double clique_linkage,
              double muw, double beta, int seed,
              deque<set<int> > &output_Eout,
              deque<deque<int> > &output_member_list,
              deque<map <int, double > >& output_Wout) {

	std::mt19937 rand(seed);
	uniform_real_distribution<double> real0to1(0,1);
    ...
    int m = clique_size;
    int n = num_nodes;
    double u = clique_linkage;

    set<int> nodes;
    set<int> active_nodes;
    set<int> deactivated_nodes;
	...
	for (int i = m; i < n; i++) {
		for (int j : active_nodes) {
			if (u > real0to1(rand) || deactivated_nodes.size() == 0) {
				Eout[i].insert(j);
				Eout[j].insert(i);
                Ein[i].insert(j);
                Ein[j].insert(i);
				...
				int no_infinite_loop = 0;
				while ((!connected) && (no_infinite_loop < 100)) {
					int sum_deactivated_degrees = 0;
    				for (auto k = node_degrees.begin() + m; k <= node_degrees.begin() + n; k++) {
						sum_deactivated_degrees += *k;
					}
					int node_j = *select_random(deactivated_nodes, rand() % deactivated_nodes.size());
					if (sum_deactivated_degrees != 0) {
						if (node_degrees.at(node_j)/sum_deactivated_degrees > real0to1(rand)) {
							if ((Eout[i].find(node_j) != Eout[i].end()) && (Eout[node_j].find(i) != Eout[node_j].end())) {
								Eout[i].insert(node_j);
								Eout[node_j].insert(i);
								Ein[i].insert(node_j);
								Ein[node_j].insert(i);
								node_degrees.at(i) += 1;
								node_degrees.at(node_j) += 1;
							}
						connected = true;
						}
					}
		...
		bool chosen = false;
		int no_infinite_loop = 0;
		while ((!chosen) && (no_infinite_loop < 100)) {
			int node_j = *select_random(active_nodes, rand() % active_nodes.size());
			if (node_degrees.at(node_j) != 0) {
				float some_sum = 0;
				for (int d : node_degrees) {
					if (d > 0) {
						some_sum += 1.0/d;
					}
				}
				float p = (1.0/node_degrees.at(node_j)) / some_sum;
				if (p > real0to1(rand)) {
					active_nodes.erase(node_j);
					deactivated_nodes.insert(node_j);
					chosen = true;
				}
			}
			no_infinite_loop += 1;
	...
	weights(Ein, Eout, member_list, beta, muw, neigh_weigh_in, neigh_weigh_out);
...
void build_network_klemm(int num_nodes, int clique_size, double clique_linkage,
                   double muw, double beta, int seed,
                   deque<set<int> > &output_Eout,
                   deque<deque<int> > &output_member_list,
                   deque<map <int, double > > &output_Wout) {
    ...
    klemm_benchmark(num_nodes, clique_size, clique_linkage, muw, beta, seed, output_Eout, output_member_list, output_Wout);
}
